 ...
 * operational lines (EGV + XOONX) since it moves every item. Sales sees only
 * EGV, XOONX only XOONX. A user with just the `history` module and no
...
export function historyScopes(a: AccessLike): Scope[] {
  ...
  if (a.canModule("order_requests")) s.add("EGV");
