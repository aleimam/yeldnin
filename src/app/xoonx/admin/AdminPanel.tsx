...
export function AdminPanel({ month, currencies, fx, categories, staff }: Props) {
  ...
  const flash = (res: { ok: boolean; error?: string }) => {
    if (res.ok) { setMsg(t("customers.saved")); setErr(null); router.refresh(); }
    else { setErr(res.error ?? "Error"); setMsg(null); }
  };

  const [rates, setRates] = useState<Record<string, string>>(
    Object.fromEntries(currencies.map((c) => [c, fx[c] != null ? String(fx[c]) : ""])),
  );
  const saveRate = (cur: string) => start(async () => flash(await setFxRateAction(month, cur, Number(rates[cur]) || 0)));

  const [newCat, setNewCat] = useState("");
  const [names, setNames] = useState<Record<number, string>>(Object.fromEntries(categories.map((c) => [c.id, c.name])));
  const addCat = () => start(async () => { const r = await createCategoryAction(newCat); if (r.ok) setNewCat(""); flash(r); });
  const rename = (id: number) => start(async () => flash(await renameCategoryAction(id, names[id] ?? "")));
  const toggle = (id: number, enabled: boolean) => start(async () => flash(await setCategoryEnabledAction(id, enabled)));
  const del = (id: number) => { if (!confirm(t("xadm.deleteCatConfirm"))) return; start(async () => flash(await deleteCategoryAction(id))); };
      ...
      <div className="card p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => router.push(`/xoonx/admin?m=${shiftMonth(month, -1)}`)} className="btn-secondary px-2 py-1">‹</button>
        ...
        <div className="space-y-2">
          {currencies.map((cur) => (
            <div key={cur} className="flex items-center gap-3">
              ...
              <button onClick={() => saveRate(cur)} disabled={pending} className="btn-secondary">{t("common.save")}</button>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="divide-y divide-line">
          {categories.map((c) => (
            <div key={c.id} className="flex flex-wrap items-center gap-2 py-2">
              <input className="input max-w-xs" value={names[c.id] ?? ""} onChange={(e) => setNames((p) => ({ ...p, [c.id]: e.target.value }))} />
              <button onClick={() => rename(c.id)} disabled={pending} className="btn-secondary">{t("common.save")}</button>
              <label className="ms-2 flex items-center gap-1 text-sm text-muted">
                <input type="checkbox" checked={c.enabled} onChange={(e) => toggle(c.id, e.target.checked)} /> {t("xadm.enabled")}
              </label>
              <button onClick={() => del(c.id)} disabled={pending} className="ms-auto text-sm text-red-600 hover:underline">{t("common.delete")}</button>
            </div>
          ))}
