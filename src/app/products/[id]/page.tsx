...
export default async function EditProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!productScopes(access, "OPERATE").includes(product.scope as never)) redirect("/products");
  const manageable = productScopes(access, "OPERATE");
  const [t, suppliers] = await Promise.all([getT(), listSuppliersForPicker()]);

  return (
    <AppShell access={access} moduleKey={primaryProductModule(access)} pageTitle={product.name} backHref="/products">
      <ProductForm
        mode="edit"
