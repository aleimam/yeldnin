...
/** Edit an existing evaluation (admins any; creator within the 14-day window).
 *  Re-scores; a rejected eval is resubmitted to Pending. */
export async function updateCsEvaluationAction(id: number, p: {
  subjectUserId: number;
  scope: string;
  typeName?: string | null;
  callDate?: string | null;
  ...
  const access = await requireUser();
  const t = await getT();
  const ev = await getEvaluationGuard(id);
  if (!ev) return { ok: false, error: t("cs.err.cantEvaluate") };
  const allowed = canEditEvaluation({ isAdmin: canManageCs(access), isEvaluator: ev.evaluatorUserId === access.user.id, createdAt: ev.createdAt });
  if (!allowed) return { ok: false, error: t("cs.err.editWindowOver") };
  if (!p.subjectUserId) return { ok: false, error: t("cs.pickRep") };
  if (p.subjectUserId === access.user.id) return { ok: false, error: t("cs.err.cantEvaluateSelf") };
  if (!p.callDate) return { ok: false, error: t("cs.pickDate") };
  const isCall = ev.scope === "CALL";
  if (isCall && !isCsChannel(p.channel)) return { ok: false, error: t("cs.pickChannel") };
  if (isCall && !p.contact?.trim()) return { ok: false, error: t("cs.enterContact") };
  const answers = (p.answers ?? []).filter((a) => a.questionId && isCsLevel(a.level));
  if (!answers.length) return { ok: false, error: t("cs.answerAll") };
  ...
  await writeAudit(access.user.id, "cs_quality", "eval.update", "csEvaluation", id, {});
  revalidatePath(`/cs-quality/evaluations/${id}`);
  revalidatePath("/cs-quality/review");
  revalidatePath("/cs-quality/submitted");
  revalidatePath("/cs-quality/mine");
  revalidatePath("/cs-quality/evaluations");
  return { ok: true, id };
}
