...
export default async function NewRequestPage() {
  ...
  const [t, products, customers, sla] = await Promise.all([getT(), listScopedProducts(allowed), listCustomerOptions(allowed), getSla()]);

  return (
    <AppShell access={access} moduleKey={primaryRequestModule(access)} pageTitle={t("requests.new")} backHref="/requests">
      <RequestForm allowedScopes={allowed} products={products} customers={customers} depositPct={sla.depositPct} />
    </AppShell>
  );
}
