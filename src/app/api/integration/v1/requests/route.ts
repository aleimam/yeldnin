...
import { handleRequestUpsert } from "@/lib/integration/request-sync";

/**
 * Veeey → YeldnIN request sync (Requests epic Phase D). Veeey's outbox POSTs the
 * wire payload here DIRECTLY (not wrapped in an envelope). 404 while the
 * integration is off; otherwise HMAC-verified, nonce-replay-protected, and
...
export async function POST(req: Request) {
  ...
  const result = await handleRequestUpsert(body);
  if (!result.ok) return err(result.skipped ?? "validation_failed", 422);

  const response = { uid: result.uid, created: !!result.created, unmatchedLines: result.unmatchedLines ?? 0 };
