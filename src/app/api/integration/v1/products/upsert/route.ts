...
import { handleProductUpsertV2 } from "@/lib/integration/product-sync";

/**
 * Veeey → YeldnIN product upsert (contract v2 §1). Veeey is the catalog-display
 * master; this SKU-keyed endpoint supersedes the legacy wpId-keyed `/catalog`
 * (which keeps running until cutover). 404 while the integration is off;
 * otherwise HMAC-verified, nonce-replay-protected, idempotent by Idempotency-Key.
 * Node runtime (Prisma + node:sqlite can't load on edge).
 */
export const runtime = "nodejs";

const ENDPOINT = "v1.products.upsert";
...
export async function POST(req: Request) {
  ...
  const result = await handleProductUpsertV2(body);
  if (!result.ok) return err(result.skipped ?? "validation_failed", 422);

  const response = { ok: true, productId: result.productId, sku: result.sku, created: !!result.created, adoptedFromWpId: !!result.adoptedFromWpId };
