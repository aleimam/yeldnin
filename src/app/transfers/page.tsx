            ...
            <tbody className="divide-y divide-line">
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-canvas/60">
                  <td className="td" data-label={t("transfers.from")}>
                    <Link href={`/transfers/${r.id}`} className="font-medium text-brand hover:underline">{r.fromName ?? `${r.fromType} #${r.fromId}`}</Link>
                  </td>
              ...
              {rows.length === 0 && <tr><td className="td text-muted" colSpan={5}>{t("transfers.empty")}</td></tr>}
            </tbody>
          </table>
        </div>
